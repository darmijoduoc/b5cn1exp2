...
public class LocationProducerController {
    ...
    public ResponseEntity<?> publishLocation(
        @Valid @RequestBody VehicleLocationDTO location
    ) {
        try {
            publisherService.publishLocation(location);
            log.info(
                "Ubicación publicada para vehículo: {}",
            ...
            log.error(
                "Error al publicar ubicación del vehículo: {}",
                location.getVehicleId(),
                e
            );
    ...
    public ResponseEntity<?> publishLocationsBatch(
        @Valid @RequestBody List<VehicleLocationDTO> locations
    ) {
        try {
            if (locations == null || locations.isEmpty()) {
                return ResponseEntity.badRequest().body(
                    Map.of(
                        ...
                        "error",
                        "message",
                        "La lista de ubicaciones no puede estar vacía"
                    )
                );
            }
