...
public class LocationConsumer {
    ...
    public void handleLocationMessage(VehicleLocationDTO location, Channel channel, Message message) {
        try {
            log.info("Mensaje de ubicación recibido para vehículo: {} en lat: {}, lon: {}",
                    location.getVehicleId(), location.getLatitude(), location.getLongitude());

            // Guardar en base de datos
            storageService.saveLocation(location);

            log.info("Ubicación guardada en base de datos para vehículo: {}",
            ...
            log.info("Ubicación procesada exitosamente para vehículo: {} (ACK enviado)",
                    location.getVehicleId());
        } catch (Exception e) {
            log.error("Error al procesar mensaje de ubicación para vehículo: {}",
                    location.getVehicleId(), e);
            try {
                // Rechazar el mensaje y reencolarlo
                channel.basicNack(message.getMessageProperties().getDeliveryTag(), false, true);
                log.warn("Mensaje reencolado para vehículo: {}", location.getVehicleId());
